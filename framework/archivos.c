...
void leerDesdeArchivo(const char *nombreArchivo, const char *formato, int num_vars, ...){
    int i;
    va_list args;
    FILE *fp = abrirArchivoR(nombreArchivo);
    if (fp == NULL){
        return;
    }
...
void escribirEnBuffer(char *buffer, size_t tamanoBuffer, const char *formato, ...){
    ...
    /* Creamos un buffer temporal para evitar que sprintf escriba directamente en el original */
    char bufferTemporal[tamanoBuffer];

    /* Escribimos el contenido en el buffer temporal */
    int longitudEscrita = vsprintf(bufferTemporal, formato, args);
